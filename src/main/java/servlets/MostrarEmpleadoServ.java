...
public class MostrarEmpleadoServ extends HttpServlet {
	...
	private PrintWriter printResponse(PrintWriter out, Map<String, String[]> parameterMap) {
		...
		res.print("<tr><th>Codigo</TH><TH>nombre</TH><TH>apellido1</TH><TH>apellido2</TH><TH>lugarnacimiento</TH></TR>");
		for (Map.Entry<Integer, Empleado> entry : listado.entrySet()) {
			Empleado val = entry.getValue();
			res.print("<tr><td>" + val.getCodigo() + "</td><td>" + val.getNombre() + "</td><td>" + val.getApellido1() 
			+"</td><td>"+ val.getApellido2() +"</td><td>"+ val.getLugarNacimiento() + "</td>");
		}
